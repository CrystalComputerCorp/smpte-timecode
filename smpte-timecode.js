// This should work both in node and in the browsers, so that is what this wrapper is about
(function (root, undefined) {
  ...
  var Timecode = function (timeCode, frameRate, dropFrame) {
    ...
    if (!(this instanceof Timecode))
      return new Timecode(timeCode, frameRate, dropFrame);
    ...
    if (typeof timeCode === "number") {
      ...
      // pick it apart
      var parts = timeCode.match(
        "^([012]\\d)[:;](\\d\\d)[:;](\\d\\d)(:|;|\\.)(\\d\\d)$"
      );
      if (!parts)
        throw new Error(
          "Timecode string expected as HH:MM:SS:FF or HH:MM:SS;FF or HH;MM;SS;FF"
        );
      ...
      this.frameCount = Math.floor(
        ((timeCode - midnight + (midnight_tz - timecode_tz)) * this.frameRate) /
          1000
      );
      this._frameCountToTimeCode();
    } else if (
      typeof timeCode === "object" &&
      typeof timeCode.hours != "undefined"
    ) {
  ...
  Timecode.prototype._validate = function (timeCode) {
    ...
    if (
      ...
      (this.dropFrame &&
        this.seconds === 0 &&
        this.minutes % 10 &&
        this.frames < this._numOfFramesToDrop())
    ) {
  ...
  Timecode.prototype.add = function (t, negative, rollOverMaxHours) {
    if (typeof t === "number") {
      ...
      if (!(t instanceof Timecode))
        t = new Timecode(t, this.frameRate, this.dropFrame);
